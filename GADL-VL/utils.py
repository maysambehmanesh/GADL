...
def compute_tversky_matrix(A1: torch.Tensor, A2: torch.Tensor, alpha=0.5, beta=0.5, eps=1e-8):
    ...
    only_i = deg1 - inter      # broadcasted [N, N]
    only_j = deg2.T - inter    # broadcasted [N, N]
...
def compute_bipartite_graph(x1,x2):
    ...
    # Nodes in V1 will be repeated for each node in V2
    V1_indices = torch.repeat_interleave(torch.arange(n1), n2)

    # Nodes in V2 will be tiled (or repeated) for each node in V1
    V2_indices = torch.arange(n1, n1 + n2).repeat(n1)

    # Stack the indices to get a 2xE tensor (2 rows: V1_indices and V2_indices)
    S = torch.stack([V1_indices, V2_indices])
...
def compute_laplacian_positional_encodings(S, X, k=10):
    ...
    # Convert to NumPy for eigen decomposition (scipy handles this better)
    L_np = L.cpu().numpy()

    # Compute k smallest eigenvectors (excluding the trivial one if needed)
...
def extract_structural_features(S, X):
    ...
    # Clustering coefficient (approximated as # triangles / total triplets)
    ...
    # Ego network size: node + neighbors
    ego_size = degree + 1
...
def farthest_point_sampling(x, n_samples):
    selected = [0]
    distances = torch.full((x.shape[0],), float('inf'), device=x.device)

    for _ in range(1, n_samples):
        last = x[selected[-1]].unsqueeze(0)
        dist = torch.norm(x - last, dim=1)
        distances = torch.minimum(distances, dist)
        selected.append(torch.argmax(distances).item())

    return torch.tensor(selected, device=x.device)



def compute_bipartite_graph_fps(x1, x2, num_samples_1, num_samples_2):
    device = x1.device
    # Apply FPS to both graphs
    idx1 = farthest_point_sampling(x1, num_samples_1)
    idx2 = farthest_point_sampling(x2, num_samples_2)

    x1_sampled = x1[idx1]
    x2_sampled = x2[idx2]

    x = torch.cat([x1_sampled, x2_sampled], dim=0)

    n1 = x1_sampled.shape[0]
    n2 = x2_sampled.shape[0]

    V1_indices = torch.repeat_interleave(torch.arange(n1, device=x.device), n2)
    V2_indices = torch.arange(n1, n1 + n2, device=x.device).repeat(n1)

    edge_index = torch.stack([V1_indices, V2_indices])  

    A = torch.zeros(n1+n2,n1+n2)
    A[V1_indices,V2_indices]=1

    return x.to(device), A.to(device), idx1.to(device), idx2.to(device)
...
def compute_graph_basis(A, k=30):
    try:
        if A.layout == torch.sparse_coo:
            A = A.to_dense()  # Convert sparse to dense
        A = A.float()

        eigval, eigvec = torch.linalg.eigh(A)  # eigvec: [N, N]

        eigval = eigval[:k]  # Eigenvalues: shape [k]
        eigvec = eigvec[:, :k]  # Eigenvectors: shape [N, k]
...
def compute_hks(eigenvecs, eigenvals, time_scales=40, eps=1e-6):
    ...
    t_min = 4.0 / l_max
    t_max = 4.0 / l_min
    ts = torch.logspace(torch.log10(torch.tensor(t_min)),
                        torch.log10(torch.tensor(t_max)),
                        steps=time_scales,
                        device=eigenvals.device)
    ...
    # Compute HKS matrix
    hks = torch.stack([
        torch.matmul(phi_sq, torch.exp(-eigenvals * t)) for t in ts
    ], dim=1)  # [N, T]
...
def MNC_regulizer(z1_h, z1_l, z2_h, z2_l, adj1, adj2, tau=0.1):
    ...
    D = torch.cdist(S_emb1, S_emb2, p=2)  # shape [N1, N2]

    P_soft = torch.softmax(-D / tau, dim=1)  # [N1, N2]
    ...
    for i in range(N1):
        ...
        neighbors_i = idx1[1][idx1[0] == i]  # [num_neighbors_i]
        if neighbors_i.numel() == 0:
            mnc_scores.append(torch.tensor(1.0, device=device))
            continue
        
        # Average of their soft match distributions → soft neighborhood match
        soft_matched_neighbors = P_soft[neighbors_i]  # [num_neighbors_i, N2]
        avg_soft_match = soft_matched_neighbors.mean(dim=0)  # [N2]

        # Current node i's own match distribution
        soft_match_i = P_soft[i]  # [N2]

        # Soft neighbors in G2 (match from P_soft[i])
        adj2_binary = torch.sparse_coo_tensor(
            idx2,
            torch.ones_like(idx2[0], dtype=torch.float32, device=device),
            size=adj2.shape,
            device=device
        ).coalesce()
        ...
        # Consistency = overlap of neighbor's average match and matched node's neighborhood
...
def spectral_mnc_loss(z1_h, z1_l, z2_h, z2_l, adj1, adj2):
    device = adj1.device
    z1 = torch.cat([z1_h, z1_l], dim=1)  # [N1, d]
    z2 = torch.cat([z2_h, z2_l], dim=1)  # [N2, d]

    z1 = F.normalize(z1, p=2, dim=1)
    z2 = F.normalize(z2, p=2, dim=1)

    S = torch.cdist(z1.double(), z2.double(), p=2)  # [N1, N2]

    M = sinkhorn_normalization(S)  # [N1, N2]

    A1 = normalize_adjacency(adj1).to_dense()  # [N1, N1]
    A2 = normalize_adjacency(adj2).to_dense()  # [N2, N2]

    M = M.float()
    A1 = A1.float()
    A2 = A2.float()

    MA2 = torch.matmul(M, A2)     # [N1, N2]
    A1M = torch.matmul(A1, M)     # [N1, N2]
